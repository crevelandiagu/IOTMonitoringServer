...
//LED Alerta
#define LED_ALERTA D5

//Constantes de luminosidad
const long A = 9000;     //Resistencia en oscuridad en KΩ
const int B = 20;        //Resistencia a la luz (10 Lux) en KΩ
...
const char ssid[] = "JOSE 602"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "Bedoya92"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "54.89.71.56"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
float humi;
// Valor de la medición de la intensidad lumínica
float ilu;
...
float readLuz() {
  ...
  long ilum = ((long)volt*A*100)/((long)B*Rc*(1024-volt)); 
 ...
 */
bool checkMeasures(float t, float h, float il) {
  // Se comprueba si ha habido algún error en la lectura
    if (isnan(t) || isnan(h) || isnan(il)) {
...
void measure() {
  if ((millis() - measureTime) >= MEASURE_INTERVAL * 1000 ) {
    ...
    if (checkMeasures(temp, humi, ilu)) {
      // Se envían los datos
      sendSensorData(temp, humi, ilu); 
    }
  }
}

void controlAlerta(String message ){
